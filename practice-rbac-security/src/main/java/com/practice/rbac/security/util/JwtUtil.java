...
import com.practice.rbac.security.confiig.JwtConfig;
    ...
    public Claims parseJWT(String jwt) {
        try {
            ...
            if (Objects.isNull(expire) || expire <= 0) {
                throw new SecurityException(String.valueOf(Status.TOKEN_EXPIRED));
            }

            // 校验redis中的JWT是否与当前的一致，不一致则代表用户已注销/用户在不同设备登录，均代表JWT已过期
            String redisToken = stringRedisTemplate.opsForValue().get(redisKey);
            if (!StrUtil.equals(jwt, redisToken)) {
                throw new SecurityException(String.valueOf(Status.TOKEN_OUT_OF_CTRL));
            }
            return claims;
            ...
            throw new SecurityException(String.valueOf(Status.TOKEN_PARSE_ERROR));
